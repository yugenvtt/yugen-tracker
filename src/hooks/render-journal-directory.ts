/**
 * @file src/hooks/render-journal-directory.ts
 * injects a launch button for the log viewer into the journal directory header.
 **/

import { LogViewer } from '../module/log-viewer.js';

export const render_journal_directory_hook = ( ) => 
{
	/** listen for the journal directory rendering to inject the log history button **/
	Hooks.on( 'renderJournalDirectory', ( _app: any, html: any, _data: any ) => 
	{
		...
		/** append the button to the journal directory header controls **/
		const root = html instanceof HTMLElement ? html : html[ 0 ];
		const header = root?.querySelector( '.directory-header .header-actions' ) || root?.querySelector( '.directory-header' ) || root?.querySelector( '.header-control' );
